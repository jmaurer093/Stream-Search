...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* HEADER */
  .header-top{display:flex;align-items:center;justify-content:space-between;padding:40px 0 24px;}
  ...
  .tab{background:transparent;border:none;border-bottom:2px solid transparent;color:var(--muted);font-family:'DM Sans',sans-serif;font-size:13px;letter-spacing:0.15em;text-transform:uppercase;padding:12px 20px;cursor:pointer;transition:all 0.2s;margin-bottom:-1px;}
  ...
  .results-label{font-size:11px;color:var(--muted);letter-spacing:0.2em;text-transform:uppercase;margin-bottom:0;padding-bottom:0;}
...
export default function StreamFinder() {
  ...
  const discover = async () => {
    ...
    try {
      ...
      if (crewId) {
        if (mediaType === "movie") params.set("with_crew", crewId);
        else params.set("with_people", crewId);
      }

      const data = await api(`/discover/${mediaType}?${params.toString()}`);
      const items = (data.results || []).slice(0, 20).map(r => ({ ...r, media_type: mediaType }));
      setDiscoverResults(items);
    } catch { setDiscoverResults([]); }
    finally { setDiscoverLoading(false); }
  };

  // Select a title and fetch providers
    ...
    <div className="service-card">
      <div className="service-header">
        {data.logo
          ...
          : <div className="service-logo-ph">▶</div>}
              ...
              {activeTab === "discover" && (
                <>
                  <div className="discover-form">
                    ...
                    <button className="search-btn" onClick={discover} disabled={discoverLoading}
                  ...
                  {!discoverLoading && discoverSearched && discoverResults.length === 0 && (
                    <div className="no-results">No titles found — try adjusting your filters.</div>
                  )}
                </>
              )}
            </>
